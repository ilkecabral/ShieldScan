...
If you are presenting this project, please note that **this is a fully functional platform**, not a simulated demo. It performs real image scans and dynamic risk scoring.

**To Run for Presentation:**
1. **Prerequisites:** Ensure Docker and Docker Compose are installed. You also need Trivy installed on your host machine if running locally (outside Docker).
2. **Environment:** Copy `app/backend/.env.example` to `app/backend/.env`. Fill in the `JWT_SECRET_KEY` and `AWS_ENCRYPTION_KEY` as documented in the file.
3. **Start the Platform:**
   ```bash
   docker-compose up -d --build
   ```
4. **Access the Dashboard:** Go to `http://localhost`. The backend API runs on port 8000.
5. **Real CWPP (Trivy) Scans:** When you hit "Run Scan" on a Docker image, it executes a live `trivy image` scan and parses the real JSON results.
6. **Dynamic Risk Scoring:** The Risk Scorer is fully operational. It dynamically calculates a 0–100 risk score based on the highest CVE severity, total volume of issues, and max CVSS score.
